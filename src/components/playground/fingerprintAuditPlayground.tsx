...
export default function FingerprintAuditPlayground() {
  ...
  const runInspection = async () => {
    if (!consented() || isInspecting()) return;
  ...
  onMount(() => {
    // Deliberately wait for explicit consent; no inspection runs on page load.
  });
    ...
    <div class="w-full max-w-5xl mx-auto flex flex-col gap-5 p-2 md:p-4 text-darkslate-100 font-sans">
      <section class="rounded-2xl border border-primary-400/30 bg-gradient-to-br from-primary-500/15 via-darkslate-700/40 to-darkslate-900/70 p-5 md:p-7 shadow-2xl shadow-black/20">
        <div class="flex flex-col gap-3">
          ...
          <p class="max-w-3xl text-sm md:text-base leading-7 text-darkslate-200">
        ...
        <div class="mt-6 rounded-xl border border-darkslate-400/50 bg-darkslate-950/50 p-4 md:p-5">
          <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between md:gap-5">
                ...
                <Show
                  ...
                  fallback={
                    isAiLoading()
                      ...
                      : "Chưa chạy audit. Hãy đồng ý để xem trình duyệt đang expose những gì."
                  }
                >
                  {aiSummary()}
                </Show>
            ...
            <Show when={snapshot()}>
              <div class="shrink-0 rounded-lg border border-primary-300/30 bg-primary-300/10 px-3 py-2 text-right">
                <div class="font-mono text-2xl font-bold text-primary-100">
                  {snapshot()?.score}
                  <span class="text-sm text-primary-300">/100</span>
                </div>
                <div class="text-[10px] uppercase tracking-widest text-primary-200">
                  exposure score
                </div>
              </div>
            </Show>
        ...
        <div class="mt-5 flex flex-col gap-3 rounded-xl border border-darkslate-400/40 bg-darkslate-900/45 p-4 md:flex-row md:items-center md:justify-between">
          <label class="flex items-start gap-3 text-xs leading-6 text-darkslate-200">
            <input
              ...
              class="mt-1 accent-primary-400"
            />
            <span>
              Tôi hiểu đây là kiểm tra trên chính trình duyệt của mình. Không
              truy cập camera, mic, vị trí, file, IP nội bộ hay gửi raw
              fingerprint đi nơi khác.
            </span>
          </label>
          <button
            type="button"
            onClick={runInspection}
            disabled={!consented() || isInspecting()}
            class="shrink-0 rounded-lg bg-primary-400 px-4 py-2.5 text-sm font-bold text-darkslate-950 transition hover:bg-primary-300 disabled:cursor-not-allowed disabled:opacity-40"
          >
            {isInspecting()
              ? "Đang kiểm tra…"
              : snapshot()
            ...
            <section class="overflow-hidden rounded-2xl border border-darkslate-500 bg-darkslate-800/35">
              <div class="flex flex-col gap-3 border-b border-darkslate-500 px-4 py-4 md:flex-row md:items-center md:justify-between md:px-5">
                ...
                <div class="flex gap-2">
                  <button
                    type="button"
                    onClick={copyReport}
                    class="rounded-md border border-darkslate-400 px-3 py-1.5 text-xs font-medium text-darkslate-200 transition hover:border-primary-300 hover:text-white"
                  >
                  ...
                  <button
                    type="button"
                    onClick={reset}
                    class="rounded-md border border-darkslate-500 px-3 py-1.5 text-xs text-darkslate-300 transition hover:border-red-300/60 hover:text-red-200"
                  >
                    Clear
                  </button>
                </div>
              ...
              <div class="divide-y divide-darkslate-600/80">
                <For each={result().signals}>
                  {(signal) => (
                    <div class="grid gap-2 px-4 py-4 md:grid-cols-[180px_minmax(0,1fr)_auto] md:items-start md:px-5">
                      <div>
                      ...
                      <div class="min-w-0">
                        <div class="break-words font-mono text-xs leading-6 text-primary-100">
                          {signal.value}
                        </div>
      ...
      <footer class="flex flex-wrap gap-x-4 gap-y-2 text-[11px] text-darkslate-500">
        <a
